...
namespace Сложныйуровень
{
    ...
    public class Task8 : Task
    {
        ...
        {
            string result = "";
            string[] words = text.Split(' '); // сплит рахделяет по проблема , в каждой ячейке хранится одно слово
            int countOfLetters = 0;// количество букв в текущей строке 
            for (int i = 0; i < words.Length; i++)
            {
                if (countOfLetters + words[i].Length <= 50)
                {
                    result += words[i] + " ";
                    countOfLetters += words[i].Length + 1;// плюс один потому что пробел
                }
                else
                {

                    for (int j = 0; j < 50 - countOfLetters; j++)
                    {
                        result += " ";
                    }
                    ...
                    result += "\n";
                    countOfLetters = 0;
                    i--;// возвращаемся к предыдущей ячейке чтобы не потерять слово 
                }
